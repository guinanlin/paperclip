...
The repo ships an initial rollout slice:

- `ServerAdapterModule` in `@paperclipai/adapter-utils` exposes optional `listSkills` / `syncSkills`; local adapters implement them with `persistent` (Codex, Cursor, Gemini, Pi, OpenCode) or `ephemeral` (Claude) semantics; `openclaw_gateway` has no server-side discovery/sync.
- Sync reconciles **Paperclip-bundled** skills by slug into each adapter’s skills home; slugs without a bundled source appear as missing in the snapshot until content import exists.
- REST: company skills, agent attachments, `GET .../skills/sync-state`, `POST .../skills/sync` (board-only for sync).
