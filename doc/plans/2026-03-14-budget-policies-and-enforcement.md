...
## Repository baseline (2026-04)

- **Schema:** `budget_policies`, `budget_incidents`; projects have `budget_lifetime_cents`, `spent_lifetime_cents`, `paused_at`, `pause_reason`.
- **Evaluation:** After each cost event, active policies are checked using **billed** cents (`cost_cents` sums excluding `subscription_included`). `calendar_month` and `lifetime` windows; scopes `company`, `agent`, `project`. Warn and hard thresholds create/update open incidents; hard stop (when enabled) pauses the agent or project and creates a pending `budget_override_required` approval linked on the incident.
- **Execution:** Heartbeat runs fail early with `project_paused` when the execution project is paused (e.g. budget).
- **API/UI:** REST for policies and incidents; Dashboard and Costs show open incidents; project overview/configuration show lifetime budget and spend.
