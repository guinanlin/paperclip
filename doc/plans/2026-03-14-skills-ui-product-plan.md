...
- **Phase A/B (partial):** Agent **Skills** tab on `AgentDetail` (sync state, attach/detach from company library, sync-to-disk when the adapter supports it). Company **Skills** page at `/skills` plus sidebar link for the skill library (slug, name, description).
- Later phases (import/export, drift across agents, external adoption) remain as in this document.
