...
export function costService(db: Db) {
  return {
    createEvent: async (companyId: string, data: Omit<typeof costEvents.$inferInsert, "companyId">) => {
      ...
      if (event.projectId && event.billingType !== "subscription_included") {
        await db
          .update(projects)
          .set({
            spentLifetimeCents: sql`${projects.spentLifetimeCents} + ${event.costCents}`,
            updatedAt: new Date(),
          })
          .where(eq(projects.id, event.projectId));
      }

      await budgetService(db).evaluateAfterCostEvent(event);
